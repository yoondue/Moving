...
public class RegexHelper {
	...
	/**
	 * 영문, 숫자, 특수문자가 하나씩 들어갔는지에 대한 형식 검사
	 * 
	 * @param str - 검사할 문자열
	 * @return boolean - 형식에 맞을 경우 true, 맞지 않을 경우 false
	 */
	public boolean isEngNumChar(String str) {
		boolean result = false;
		if (isValue(str)) {
	 ...
	 * @return boolean - 형식에 맞을 경우 true, 맞지 않을 경우 false
	 */
	public boolean isEngKorNum(String str) {
		boolean result = false;
		if (isValue(str)) {
			result = Pattern.matches("^[ㄱ-ㅎ|가-힣|a-z|A-Z|0-9|\\*]+$", str);
		}
		return result;
	}
